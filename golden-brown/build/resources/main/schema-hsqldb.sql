...
CREATE TABLE CLICK(
	...
	IP			VARCHAR(20),				-- IP
	COUNTRY		VARCHAR(50),				-- Country
	CONSTRAINT FK_CLICK FOREIGN KEY(HASH) REFERENCES SHORTURL(HASH)
);
...
CREATE TABLE USERS(
	...
	PASSWORD	VARCHAR(45) NOT NULL,				-- Contrase�a
...
CREATE TABLE MULTIPLESURIS(
	HASH			VARCHAR(30) NOT NULL ,						-- Key, id original
	TARGET			VARCHAR(1024) NOT NULL ,			-- Key, destino
	EXPRESSION		VARCHAR(1024) NOT NULL,						-- Regular expression
	CONSTRAINT FK_MULTIPLES_URIS FOREIGN KEY(HASH) REFERENCES SHORTURL(HASH),
	PRIMARY KEY (HASH, TARGET, EXPRESSION)
);
