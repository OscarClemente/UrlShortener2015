...
@Repository
public class ShortURLRepositoryImplExtended implements ShortURLRepositoryExtended {
	...
	private static final RowMapper<ShortURL> rowMapper = new RowMapper<ShortURL>() {
		...
		public ShortURL mapRow(ResultSet rs, int rowNum) throws SQLException {
			return new ShortURL(rs.getString("hash"), rs.getString("target"),
					...
					rs.getBoolean("advert"), rs.getInt("seconds"));
		}
	};
	...
	@Override
	public ShortURL findByHash(String hash, String username) {
		try {
			return jdbc.queryForObject("SELECT * FROM shorturl WHERE hash=? AND username=?",
	...
	public ShortURL save(ShortURL su) {
		try {
			jdbc.update("INSERT INTO shorturl VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
					su.getHash(), su.getTarget(), su.getSponsor(), su.getCreated(),
					su.getOwner(), su.getMode(), su.getSafe(), su.getIP(), su.getCountry(),
					su.getUsername(), su.getAdvert(), su.getSeconds());
	...
	public void update(ShortURL su) {
		try {
			jdbc.update(
					"update shorturl set target=?, sponsor=?, created=?, owner=?, mode=?, safe=?, ip=?, country=?, advert=?, seconds=? where hash=?",
					su.getTarget(), su.getSponsor(), su.getCreated(),
					su.getOwner(), su.getMode(), su.getSafe(), su.getIP(),
					su.getCountry(), su.getHash(), su.getAdvert(), su.getSeconds());
		} catch (Exception e) {
	...
	@Override
	public void deleteByHash(String username, String hash, String target) {
		try {
			jdbc.update("DELETE FROM shorturl WHERE hash=? AND target=? AND username=?", hash, target, username);
		} catch (Exception e) {
			log.debug("When delete for hash " + hash, e);
		}
	}
